  ...
  if (router) {
    router.beforeEach((to, from, next$$1) => {
      ...
      if (matchedComponents.length) {
        let arr = [];
        getComponentsDepth(Vue$1, matchedComponents, deepth, arr);
        let payloads = [];
...
function getComponentsDepth(Vue$$1, components, depth, arr) {
  if (Array.isArray(components)) {
    for (let i = 0; i < components.length; ++i) {
      appendComponent(Vue$$1, components[i], depth, arr);
    }
  } else {
    for (let comName in components) {
      appendComponent(Vue$$1, components[comName], depth, arr);
    }
  }
  return arr;
}

function appendComponent(Vue$$1, com, depth, arr) {
  if (isString(com)) {
    com = Vue$$1.component(com);
  }
  if (com) {
    arr.push(com);
    if (depth && com.components) {
      getComponentsDepth(Vue$$1, com.components, depth--, arr);
    }
  }
}
